...
pub mod serde_address_type {
    ...
    pub fn serialize<S>(status: &i32, serializer: S) -> Result<S::Ok, S::Error> where S: Serializer {
    ...
    pub fn deserialize<'de, D>(deserializer: D) -> Result<i32, D::Error> where D: Deserializer<'de> {
        ...
        impl<'de> de::Visitor<'de> for StatusVisitor {
            ...
            fn visit_str<E>(self, v: &str) -> Result<Self::Value, E> where E: de::Error {
                let status = AddressType::from_str(v).map_err(|_|
                    E::custom("given address type is unknown")
                )?;
                Ok(status as i32)
            }
        }
