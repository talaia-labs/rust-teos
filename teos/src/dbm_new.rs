...
use bitcoin::{consensus, BlockHash, Txid};
...
impl DBM {
    ...
    pub(crate) async fn load_penalties_summaries(
        &self,
    ) -> HashMap<UUID, (Txid, ConfirmationStatus)> {
...
mod tests {
    ...
    impl DBM {
        ...
        async fn load_user(&self, user_id: UserId) -> Option<UserInfo> {
    ...
    async fn test_batch_check_locators_exist() {
        ...
        for appointment in appointments.iter() {
            dbm.store_appointment(
                UUID::new(appointment.locator(), appointment.user_id),
                appointment,
            )
            .await
            .unwrap();
    ...
    async fn test_update_tracker_status() {
        ...
        assert!(matches!(
            dbm.update_tracker_status(uuid, &ConfirmationStatus::Rejected(100))
                .await,
            Err(Error::Decode(..))
        ));
    }
    ...
    #[tokio::test]
    async fn test_load_trackers_with_confirmation_status() {
        let dbm = DBM::test_db().await;
        let n_trackers = 100;
