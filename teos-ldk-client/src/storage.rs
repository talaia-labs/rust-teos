...
/// Trait for types that can be serialized to and from bytes
pub(crate) trait Serializable {
    /// Convert the type to a byte vector
    fn to_vec(&self) -> Result<Vec<u8>, io::Error>;

    /// Create an instance from a byte slice
    fn from_slice(slice: &[u8]) -> Result<Self, io::Error> where Self: Sized;
}

impl Serializable for TowerInfo {
    fn to_vec(&self) -> Result<Vec<u8>, io::Error> {
